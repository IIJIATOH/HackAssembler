...
func MyFunction() {
	fmt.Println("MyFunction called")
	read()
}

func read() {
	// Открываем файл
	file, err := os.Open("C:/Users/mikhailovpa.DESKTOP-OKO95JV/Desktop/ассемблер/filesToRead/Pong.asm")
	if err != nil {
		log.Fatalf("failed to open file: %s", err)
	}
	defer file.Close()
	file2, err2 := os.Open("C:/Users/mikhailovpa.DESKTOP-OKO95JV/Desktop/ассемблер/filesToRead/Pong.asm")
	if err2 != nil {
		log.Fatalf("failed to open file: %s", err)
	}
	defer file2.Close()
	...
	for scannerSecond.Scan() {
		...
		if strings.HasPrefix(trimLine, "@") {
			...
			fmt.Println(binaryDest)
			// Обрабатываем C- инструкцию без символов
		}
